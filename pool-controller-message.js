...
var message = function PoolControllerMessage( /*Buffer*/ buffer ) {
  var source, dest, msgLength, data, checksum, offset, checksumOffset, hour, minute, preamble;
  ...
  if ( buffer ) {
    source = _detectEquipment( buffer[5] );
    dest = _detectEquipment( buffer[4] );
    msgLength = buffer[7];
    offset = 8;
  ...
  return {
    ...
    */
    toBuffer: function() {
      ...
      var buffNoCheck = Buffer.concat( [ preamble, bytes, this.dataBuffer ],
        preamble.length + bytes.length + this.dataBuffer.length );
